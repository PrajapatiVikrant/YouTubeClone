...
const ChannelPage = () => {
  ...
  async function getVideos() {
    ...
    try {
      setLoading(true)
      const response = await axios.get(`http://localhost:4000/api/channel/${id}`, {
        headers: {
      ...
      const res = await axios.get(
        `http://localhost:4000/api/video/channel/${response.data._id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleSaveEdit = async (video) => {
    ...
    const res = await axios.put(
      `http://localhost:4000/api/video/${video._id}`, video,
      {
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
  ...
  const handleDelete = async (id) => {
    ...
    const token = localStorage.getItem("token");
    try {
      await axios.delete(`http://localhost:4000/api/video/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
