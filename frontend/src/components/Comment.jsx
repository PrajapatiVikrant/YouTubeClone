...
export default function Comment({ comments, videoId }) {
  ...
  async function addComment() {
    ...
    setLoading(true)
    const res = await axios.post(`http://localhost:4000/api/comment/${videoId}`, { text: commentMessage }, {
      headers: {
  ...
  async function editComment(commentId, text) {
    ...
    try {
      const res = await axios.put(
        `http://localhost:4000/api/comment/${videoId}/${commentId}`,
        { text },
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  async function DeleteComment(commentId) {
    const token = localStorage.getItem('token')
    const res = await axios.delete(`http://localhost:4000/api/comment/${videoId}/${commentId}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    })
